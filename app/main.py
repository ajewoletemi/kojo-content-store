...
def upload_file_to_supabase(file: UploadFile, folder: str = "delivery") -> str | None:
...
def upload_image_to_supabase(file: UploadFile) -> str | None:
    """Upload image to Supabase (for product images)"""
    return upload_file_to_supabase(file, folder="images")
...
@app.get("/top-up", response_class=HTMLResponse)
async def top_up_page(request: Request, user=Depends(require_user)):
    return render(request, "top_up.html", user=user)


@app.post("/top-up")
async def top_up_submit(amount: float = Form(...), user=Depends(require_user), db: Session = Depends(get_db)):
    if amount < 10:
        return render(request, "top_up.html", {"error": "Minimum top-up is $10"}, user=user)
    ...
    order = Order(
        ...
        status="pending",
        payment_type="btc",
        notes=f"Store Credit Top-up of ${amount:.2f}",
        custom_title="Store Credit Top-up"
    )
    db.add(order)
    db.commit()
    db.refresh(order)
...
async def place_custom_order(service_id: int, details: str = Form(...), user=Depends(require_user), db: Session = Depends(get_db)):
    ...
    if (user.credits or 0) >= service.price_usd:
        ...
        order = Order(
            user_id=user.id, product_id=None, amount_usd=service.price_usd,
            status="paid", payment_type="credits", notes=details.strip(), custom_title=service.title
        )
        db.add(order)
        db.commit()
        db.refresh(order)
    ...
    order = Order(
        user_id=user.id, product_id=None, amount_usd=service.price_usd,
        status="pending", payment_type="btc", notes=details.strip(), custom_title=service.title
    )
    db.add(order)
    db.commit()
    db.refresh(order)
...
async def download_file(order_id: int, user=Depends(require_user), db: Session = Depends(get_db)):
    ...
    # 1. Delivery file stored on Supabase (public URL)
    if order.delivery_file:
        if order.delivery_file.startswith("http"):
            return RedirectResponse(order.delivery_file)
        # old local fallback
        local_path = UPLOAD_DIR / "delivery" / order.delivery_file
        if local_path.exists():
            return FileResponse(path=local_path, filename=local_path.name)

    # 2. Product digital file (local)
    if order.product and order.product.file_path:
...
async def deliver_order(
    ...
    # Upload delivery file permanently to Supabase
    if delivery_file and delivery_file.filename:
        public_url = upload_file_to_supabase(delivery_file, folder="delivery")
        if public_url:
            order.delivery_file = public_url
